package frc.robot.lib.Pixy2;

import java.awt.Color;
import java.util.concurrent.TimeUnit;

import frc.robot.lib.Pixy2.links.Link;
...
public class Pixy2 {
	...
	public int init(int argument) {
		...
		if (ret >= 0) {
			...
			for (long t = System.currentTimeMillis(); System.currentTimeMillis() - t < 5000;) {
				// Gets version and resolution to check if communication is successful and
	...
	/**
	 * Initializes Pixy2 and waits for startup to complete using default link
	 * argument value
	 * 
	 * @return Pixy2 error code
	 */
	...
	public byte changeProg(char[] prog) {
		...
		while (true) {
			// Truncates supplied program name, or adds empty characters after to indicate
			// end of string
